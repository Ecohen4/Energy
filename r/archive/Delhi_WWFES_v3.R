###### REVISED for ERL publicatoin on 2015-03-01
...
CGS$Date <- as.Date(paste(CGS$year, CGS$month_id, '15', sep='-'))

## combine multiple generating units at the same location (stn_code coerced to match in Excel)
## UNITS = LU = 10^5 KWH = 0.1 GWH
CGS <- ddply(CGS,.(Date, year, month_id, stn_code, seb_code), numcolwise(sum))

## plot CGS allocatsion to Delhi
CGSplot <- ggplot(CGS, aes(x=Date, y=t_energy_month/10)) +
  geom_line() +
  facet_wrap(~stn_code, scale="free") +
...
## cumulative stationwise energy allocations from CGS to Delhi
zz <- ddply(CGS,.(stn_code), summarize, t_energy = sum(t_energy_month))
## THESE MATCH PRINTOUT FROM NRPC! UNITS = LU = 10^5 KWH = 0.1 GWH

## keep CGS with non-zero contributions to Delhi (>0 LU/yr)
keep <- subset(zz, t_energy>0)
keep.names <- keep$stn_code
dim(keep)[1]  #28 CGS with non-zero energy allocations to Delhi in 2011-2012
# NOTE: this is after combining multiple units at the same location:
# ANT = ANTG + ANTL + ANTR
# AUR = AURG + AURL + AURR
# DDR = DDRG + DDRL + DDRR
keep.data <- CGS[CGS$stn_code %in% keep.names,]
keep.data <- droplevels(keep.data) #drops unused factor levels
#keep.data$stn_code <- as.factor(keep.data$stn_code)
#keep.data$seb_code <- as.factor(keep.data$seb_code)
str(keep.data)

drop <- subset(zz, t_energy==0)
drop.names <- drop$stn_code
dim(drop)[1]
drop.data <- CGS[CGS$stn_code %in% drop.names,]
drop.data <- droplevels(drop.data)
#drop.data$stn_code <- as.factor(drop.data$stn_code)
#drop.data$seb_code <- as.factor(drop.data$seb_code)
str(drop.data)

# Repeat above procedure to double-check ##
intersect <- function(x, y) y[match(x, y, nomatch = 0)]
index <- intersect(CGS$stn_code, keep.names)
keep.data2 <- subset(CGS, CGS$stn_code %in% index)
keep.data2 <- droplevels(keep.data2)
identical(keep.data,keep.data2) # TRUE

# Check if we captured all the data
identical(dim(keep.data)[1]+dim(drop.data)[1], dim(CGS)[1])

## format and replace CGS with cleaned version (2015-03-01)
keep.data$stn_code <- as.factor(keep.data$stn_code)
keep.data$seb_code <- as.factor(keep.data$seb_code)
CGS <- keep.data

save(keep.data, file="keep.data.rdata")
save(CGS, file="CGS.rdata") # CGS allocations from grid to Delhi CLEAN
#############################
...
load("CGS.rdata") # CGS allocations from grid to Delhi CLEAN
load("keep.data.rdata") # IDENTICAL
...
compare <- data.frame(stn_code = merge$stn_code,
                      Date = merge$Date,
                      Allocations_to_NR_States = apply(merge[,19:27], 1, sum),
                      Capacity_MW = merge$Capacity_MW,
                      Pct_Allocated = merge$Pct_Allocated
                      )
compare$Pct_Check <- compare$Allocations_to_NR_States / compare$Capacity_MW
head(compare)
...
# keep.cols <- c("stn_code", "Station", "Fuel", "Capacity_MW", "Lat", "Long", "X", "Y", "State", "Date", "year", "month_id", "t_energy_month", "Allocations_to_NR_States", "Pct_Allocated", "Km_to_Delhi", "Wgt_Distance_to_Delhi", "PLF")
#
# merge<-merge[,-c(19:28)]
...
title<-"Trans-Boundary Water Footprint\nof Energy Supplied From Grid to Delhi, Disaggregated by Source\n(April 2011 to Feb 2013)"

p3 <- ggplot(merge,aes(x=Date,y=WF,colour=metric,linetype=stat)) +
  geom_line() +
  facet_wrap(~stn_code, scale="free_y", nrow=4) +
...
p3mean <- ggplot(mean, aes(x=Date, y=WF, colour=metric)) +
  geom_line() +
  facet_wrap(~stn_code, scale="free_y", nrow=4) +
  scale_x_date(labels = date_format("%b")) +
  labs(title=title, y="Million Liters Freshwater per Month", x="Month") +
  theme_minimal() +
