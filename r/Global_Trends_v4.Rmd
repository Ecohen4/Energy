...
```{r urbanization_map_evolution, fig.width=10, fig.height=5, eval=FALSE}
# base map
world <- map_data("world")
worldmap <- ggplot(world, aes(x=long, y=lat, group=group)) +
  geom_path()
...
The 1st step was to collect high resolution (hourly) load, climate and population data for as many cities as possible.

The starting point for emerging megacities was the UN World Urbanization Prospects 2014 list of cities, subsetted by the 100 cities of more than 2M habitants with the highest population growth rate for the next 5 years (2015-2020). Concerning cities in the North, data for US cities were collected from the Federal Energy Regulatory Commission and US Energy Information Administration databases. 


| City        | Country             | Load          | Weather       | Population    |
|-------------|---------------------|---------------|---------------|---------------|
|             |                     | Years (ref)   | Years (ref)   | Years (ref)   |
|             |                     |---------------|---------------|---------------|                     
...
### Weather Data
The National Oceanic and Atmospheric Administration (NOAA) offers a variety of free meteorological and climatological information through the National Climatic Data Center. The data is available on the Online Climate Data Directory website, available via FTP, which can be downloaded and efficiently processed in R. The closest weather stations to cities of interest were chosen, and invalid data (missing values, 999s and NAs) were either interpolated or omitted. The final set of hourly data collected is: Temperature, Wind Speed, Wind Direction, Dew Point Temperature and Atmospheric Pressure. 

### Demand Data
High resolution hourly load data for the past 4 years (on average - could be less or more for some cities) was collected from utility companies, system operators and electricity regulatory bodies.

Data was obtained for 21 US cities **(check number)** and 13 emerging megacities **(check number)** in the Tropics/Subtropics. 

### Population Data
Population data was collected from the UN World Urbanization Prospects 2014 edition and from the US Bureau of Economic Analysis. It is used to normalize energy demand and consumption to a per capita level, as per the *Method* section. 
...
Because there is usually reduced economic and commercial activities on weekends compared to weekdays, demand for electricity is expected to be lower on weekends. To avoid this noise in the analyses, corresponding observations are omitted. 
...
```{r,echo=FALSE,fig.width=3, fig.height=3, message=FALSE,warning=FALSE}
...
**Most recent Cooling/Heating Demand**
```{r, results = 'asis'}
...
**Most recent Cooling/Heating Energy**
```{r, results = 'asis'}
