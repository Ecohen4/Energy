...
par(mfrow=c(1,2))
t <- plot(nyc$temperature,nyc$mw, main="Segmented regression for NYC", xlab="Temperature (DegC)", ylab="Peak Demand (MW)")+plot(segmented.nyc,add=TRUE,res=FALSE,link=TRUE,col="green",size=0.9)

q <- plot(dakar$temperature,dakar$mw, main="Linear regression for Dakar", xlab="Temperature (DegC)", ylab="Peak Demand(MW)") + abline(linear.dakar,col="green")
...
```{r population-reshape, echo=FALSE, message=FALSE}
...
**Result 1:** City-Scale peak demand for electrical heating and cooling [MW/(∆T)] 
```{r sparkTable, results = 'asis', eval=FALSE}
...
#     function(x) { paste(min(x), max(x), sep=" / " )}, # range heating demand 
#     newSparkLine(height = 0.25, width = 1, pointWidth = 2, lineWidth = 1),           # heating demand timeseries
#     function(x) { paste(min(x), max(x), sep=" / " )}, # range cooling demand 
#     newSparkLine(height = 0.25, width = 1, pointWidth = 2, lineWidth = 1)            # cooling demand timeseries
#     )
# 
# names(content) <- c(" Year Range ","Heating Range min/max [MW/deltaT]","Heating Trend"," Cooling Range min/max [MW/deltaT] ","Cooling Trend") 
...
![Caption for the picture.](latex_tables/peak-absolute.png)

**Result 2:** Per-capita peak demand for electrical heating and cooling [MW/(∆T x capita)] 
```{r sparkTable.percap, results = 'asis', eval=FALSE}
...
![Caption for the picture.](latex_tables/peak-capita.png)

**Result 3:** Per-Capita integral energy consumption for heating & cooling [kWh/(capita x yr)]
```{r, results = 'asis'}
energy <- test1
pop <- population
...
**Result 4:** Normalized per-capita integral energy consumption for heating & cooling [Fraction of Total]
```{r normalized.cooling.share, results='asis'}
...
**Result 5:** Include Nairobi for increased heating and Abidjan for increased Cooling
