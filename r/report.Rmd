...
Building energy demand for thermal comfort (in particular) has been evaluated for many regions of the world:
...
Within the OECD set, there is substantial variation. Mid-size, relatively sprawling, U.S. cities such as Detroit, MI; Chattanooga, TN; and Omaha, NE; appear to have the highest per-capita demand for cooling, at approximately 100 W/(capita x °C). Population dense New York City, on the other hand, has a cooling demand of just 40 W/(°C x capita) despite having seven times as many standardized CDH as Detroit, five times as many as Omaha, and three times as many as Chattanooga. In fact, Singapore has the highest number of standardized CDH of any city in our study, with over 70,000 per annum. Table 5 provides a summary table of per-capita peak demand for electrical cooling (and heating) alongside standardized and optimized CDH (HDH) for all cities in our study. 
...
Several previous studies have estimated urban energy requirements for heating and cooling. Most relevant, Chaturvedi et. al. (2014) simulate building energy demand for India out to 2095. They take a top-down approach, applying national-average data on residential-commercial asset ownership and building square-footage as inputs for a building energy service model sub-routine (Eom et. al. 2012) of the Global Change Assessment Model (GCAM; Edmonds et. al. 1997). Modeling is performed seperately for urban and rural buildings given divergent baseline conditions and trajectories. Broadly, Chaturvedi et. al. (2014) highlight the same themes as us: rapid urbanization and demand for building energy services are key drivers of global energy demand. However, our specific findings for urban India differ substantially, as detailed below.

For benchmark comparison, we harmonize our city-specifc results for Delhi and Chanidargh (India) with all-India urban averages from Chaturvedi et. al. (2014). We estimate per-capita electricity consumption for cooling to be 370 kWh/(capita x year) for NCT Delhi (see Table 5)[^1]. Applying values reported in Chaturvedi et. al. (2014) and scaling by the number of households in Delhi (Chavez et al. 2012) and the average number of people per household (Delhi Statistical Abstract 2012), their estimate is equivalent to 36 kWh/(capita x year) -- an order of magnitude smaller than ours.

Similarly, bootstrapping Chaturvedi et. al. (2014) estimates, the total cooling load for Delhi would be equivalent to 423 MW on an average cooling-day. By comparison, we find peak cooling load for Delhi to be 5359 MW, as per Table 6. While our estimate is for peak cooling demand, and Chaturvedi (2014) is for average cooling demand on a cooling day, the magnitude of difference cannot be explained by this alone. Assuming an average cooling day in Delhi is just 10 °C above a 21 °C threshold temperature, that would bring our estimate down to 1300 MW (see result 1) -- still three times higher than Chaturvedi. One interpretation is that A/C ownership in Delhi is three times higher than the national average for urban areas in India.
...
**Table 5:** Summary Table of Per-Capita Peak Demand and Integral Energy Consumption for Thermal Comfort
...
```{r, results='asis'}
pander(summary.t, split.cells = 17, split.table = Inf)
# kable(summary.t, digits = 2)
```

*Footnote: Optimized HDH/CDH are calculated from the derived city-specific Threshold Temperature. Standard HDH/CDH are calculated assuming a 20°C baseline Threshold Temperature*

\newpage

**Table 6:** Urban Peak Load Analysis
